...
class TaskMonitor:
    ...
    def download_deals(self):
        ...
        result = self.b.get_all(
            "crm.deal.list",
            params={
                "filter": {"CLOSED": "N"},  # берем только сделки в работе
                "select": ["ID", "ASSIGNED_BY_ID"],
            },
        )
        self.deals = {d["ID"]: {"ASSIGNED_BY_ID": d["ASSIGNED_BY_ID"]} for d in result}

    def download_activities(self):
        log("Downloading activities")
        self.activities = self.b.call(
            "crm.activity.list",
            [
                {
                    "filter": {
    ...
    def keep_only_deals_without_tasks(self):
        ...
        self.deals = {
            ID: contents for ID, contents in self.deals.items() if ID not in owner_IDs
        }
    ...
    def download_contacts_details(self):
        ...
        self.contacts = self.b.get_by_ID(
            ...
            ID_list=[
                contents["CONTACT_ID"]
                for contents in self.deals.values()
                if "CONTACT_ID" in contents
            ],
            params={"select": ["ID", "PHONE", "NAME", "LAST_NAME"]},
        )
    ...
    def add_phones_to_deals(self):
        ...
        for deal_ID, contents in self.deals.items():
            ...
            if (
                "CONTACT_ID" in contents
                and contents["CONTACT_ID"] in contacts_processed
            ):
                self.deals[deal_ID]["PHONE"] = contacts_processed[
                    contents["CONTACT_ID"]
                ]
